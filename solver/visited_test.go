...
func TestAdd(t *testing.T) {
	visited := initVisited()
	add(visited, [48]int{0, 1, 2, 3}, []string{"U"})
	add(visited, [48]int{0, 1, 2, 3}, []string{"F"})
	add(visited, [48]int{4, 5, 6, 7}, []string{"D"})

	movesArray := get(visited, [48]int{0, 1, 2, 3})
	if len(movesArray) != 2 {
	...
	movesArray = get(visited, [48]int{4, 5, 6, 7})
	if len(movesArray) != 1 {
